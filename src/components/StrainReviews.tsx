...
const StrainReviews: React.FC<StrainReviewsProps> = ({
  strainId,
  strainName
}) => {
  const {
  ...
  const averageRating = reviews.length > 0 ? Math.round(reviews.reduce((acc, review) => acc + review.rating, 0) / reviews.length * 10) / 10 : 0;
  ...
  return <div className="space-y-6">
      {/* Reviews Header */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 px-0 mx-0">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-semibold text-foreground">Reviews</h2>
            {reviews.length > 0 && <span className="flex items-center">
                <span className="flex text-yellow-400 mr-1">
                  {renderStars(Math.round(averageRating))}
                </span>
              </span>}
          </div>
          {reviews.length > 0 && <p className="text-muted-foreground text-sm mt-1">
              Average Rating: {averageRating} 
            </p>}
        </div>
        
        <Button onClick={handleAddReviewClick} className="bg-emerald-600 hover:bg-emerald-700 w-full sm:w-auto mx-0 px-[22px]">
          ...
          <DialogContent className="bg-card text-foreground border-primary/20">
            ...
            <div className="space-y-4 pt-4">
              ...
              <div>
                ...
                <Textarea value={userReview} onChange={e => setUserReview(e.target.value)} className="bg-background border-input text-foreground" rows={4} placeholder="Share your experience with this strain..." />
              </div>
              ...
              <Button onClick={handleSubmitReview} disabled={userRating === 0} className="w-full bg-emerald-600 hover:bg-emerald-700">
                Submit Review
              </Button>
            </div>
          </DialogContent>
        ...
        <UsernamePrompt isOpen={isUsernamePromptOpen} onClose={() => setIsUsernamePromptOpen(false)} onUsernameSaved={handleUsernameSaved} />
      </div>
      
      {reviews.length > 0 ? <div className="space-y-4">
          {reviews.map(review => <Card key={review.id} className="hover:border-primary/30 transition-all duration-300">
              ...
              <CardContent>
                ...
                {review.photoUrl && <div className="mt-3">
                    <img src={review.photoUrl} alt="Review" className="rounded-md max-h-40 object-cover" />
                  </div>}
              </CardContent>
            </Card>)}
        </div> : <Card className="text-center p-8 border-primary/20">
